...
export function db(){if(!env.DB)throw new AppError('The record store is not available. Please try again.',503);return env.DB}
export function bucket(){if(!env.FILES)throw new AppError('File storage is not available. Please try again.',503);return env.FILES}
export async function session():Promise<Session>{
 const user=await getChatGPTUser();if(!user)throw new AppError('Sign in to open your workspace.',401);
...
export function checkOrigin(request:Request){const origin=request.headers.get('origin');if(origin&&origin!==new URL(request.url).origin)throw new AppError('This request must come from your workspace.',403)}
