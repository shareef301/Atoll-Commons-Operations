...
export async function POST(request:Request){let key:string|undefined;try{checkOrigin(request);const s=await session();const form=await (await boundedRequest(request,11*1024*1024)).formData();const f=form.get('file');const projectId=String(form.get('projectId')||'');const scope=String(form.get('scope')||'organization');
 ...
 const savedScope=s.access.role==='Treasurer'&&scope==='organization'?'finance':scope;
 if(!fileAccess(s,{projectId,scope:savedScope},true))throw new AppError('You cannot add evidence to this scope.',403);
 const allowed=/\.(pdf|docx|xlsx|csv|txt|md|png|jpe?g|webp|svg|zip)$/i;if(!allowed.test(f.name))throw new AppError('Use PDF, DOCX, XLSX, CSV, text, PNG, JPEG, WebP, SVG or ZIP.');
 ...
 s.data.files.push({id,title:f.name.replace(/[\r\n]/g,'').slice(0,200),status:'Retained',projectId,scope:savedScope,size:f.size,mime:f.type,sha256:checksum,createdAt:now(),owner:s.user.email,key});s.data.audit.unshift({id:uid(),at:now(),actor:s.user.email,action:'Evidence retained',detail:f.name,recordId:id,...(['dues','exco'].includes(savedScope)?{privacy:savedScope as 'dues'|'exco'}:{})});await save(s,uid());return Response.json({...visible(s),fileId:id,message:'Evidence saved securely.'});
