...
export async function POST(request:Request){let key:string|undefined;try{checkOrigin(request);const s=await session();const form=await request.formData();const f=form.get('file');const projectId=String(form.get('projectId')||'');const scope=String(form.get('scope')||'organization');
 ...
 }catch(e){if(key)await bucket().delete(key).catch(()=>{});return responseError(e)}}
