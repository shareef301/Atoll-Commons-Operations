...
export default async function SignIn({searchParams}:{searchParams:Promise<{error?:string}>}) {
  ...
  return <main className="signin-page"><section className="signin-card">
    ...
    {local ? <form action="/auth/development" method="post"><button className="button primary" type="submit">Open local preview</button><p className="metadata">Local development only · sample workspace</p></form> : process.env.AUTH_MODE==='supabase'?<><EmailSignIn/>{process.env.SUPABASE_GOOGLE_ENABLED==='true'&&<a className="button secondary" href="/auth/google">Continue with Google</a>}</>:<a className="button primary" href="/auth/google">Continue with Google</a>}
